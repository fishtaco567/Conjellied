...
namespace Entities {
    ...
    public class CharacterController2D : MonoBehaviour {
        ...
        public CharacterRaycastOrigins _raycastOrigins2;
        public CharacterRaycastOrigins _raycastOrigins3;
        ...
        public void recalculateDistanceBetweenRays() {
            ...
            // horizontal
            var colliderUseableHeight = boxCollider.size.y * Mathf.Abs(transform.localScale.y) - (2f * skinWidth2);
            _verticalDistanceBetweenRays = colliderUseableHeight / (totalHorizontalRays - 1);

            // vertical
            var colliderUseableWidth = boxCollider.size.x * Mathf.Abs(transform.localScale.x) - (2f * skinWidth2);
        ...
        void primeRaycastOrigins() {
            ...
            _raycastOrigins2.topLeft = new Vector2(modifiedBounds.min.x + skinWidth, modifiedBounds.max.y - skinWidth2);
            _raycastOrigins2.bottomRight = new Vector2(modifiedBounds.max.x - skinWidth, modifiedBounds.min.y + skinWidth2);
            _raycastOrigins2.bottomLeft = new Vector2(modifiedBounds.min.x + skinWidth, modifiedBounds.min.y + skinWidth);

            _raycastOrigins3.topLeft = new Vector2(modifiedBounds.min.x + skinWidth2, modifiedBounds.max.y - _skinWidth);
            _raycastOrigins3.bottomRight = new Vector2(modifiedBounds.max.x - skinWidth2, modifiedBounds.min.y + _skinWidth);
            _raycastOrigins3.bottomLeft = new Vector2(modifiedBounds.min.x + skinWidth2, modifiedBounds.min.y + _skinWidth);
        }
        ...
        float moveHorizontally(ref Vector3 deltaMovement) {
            ...
            var initialRayOrigin = isGoingRight ? _raycastOrigins2.bottomRight : _raycastOrigins2.bottomLeft;
            float angle = 0;
        ...
        void moveVertically(ref Vector3 deltaMovement, float angle) {
            ...
            var initialRayOrigin = isGoingUp ? _raycastOrigins3.topLeft : _raycastOrigins3.bottomLeft;
            ...
            for (var i = 0; i < totalVerticalRays; i++) {
                ...
                if (numHits != 0) {
                    ...
                    if(rayResults[0].collider.gameObject.layer == 12) {
                        var num = Physics2D.OverlapPointNonAlloc(ray, results, oneWayPlatformMask);
                        if(num != 0) {
                            continue;
                        }
                    }
